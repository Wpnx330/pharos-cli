...
| W5.4 | A7-MCP pharos serve (MCP stdio) | 🟡 BUILT (Sep 8, this branch) — internal/mcpserver: hand-rolled JSON-RPC 2.0 stdio server (zero new go.mod deps, mcpclient framing discipline); methods initialize (protocolVersion 2024-11-05, serverInfo pharos+cmd.Version), notifications/initialized (silent ack), tools/list, tools/call, ping; errors -32601 unknown method / -32602 bad params / -32700 parse / -32600 invalid request+batch-unsupported, tool failures = isError tool results (never crash, panic recovery), null-id requests treated as notifications, stdout purity (diagnostics → stderr only); tools search (limit clamp 1..50) / info (install_hint) / list_installed (legacy origin backfill "registry", clients never null) + install opt-in via --allow-install (omitted from tools/list when off, direct call → isError naming the flag; enabled = full install pipeline: semver resolve, store, canonical config w/ idle 60, detected clients, pharos.lock origin InstalledVia "pharos serve", dep resolution warnings); 25+ tests: in-process unit (protocol errors, purity, ctx-cancel, panic) + subprocess e2e (handshake, tools/list 3-vs-4, search/info vs httptest fixture, error codes, strict-JSON transcript purity, stdin-EOF shutdown w/ no extra frames); gates: build/vet/test, -race -count=2, GOOS=windows build+vet, GOOS=darwin build all green | app dev | pharos-cli (this branch) |
| W5   | C2 / A5 (A6→W5.1, C2→W5.2, A5→W5.3 shipped; A7→W5.4 BUILT) | 🟡 in-flight | app dev | pharos-cli |
