...
| site | Wave G cfdecd5: v2.0.0 parity live (8 groups, 3 surfaces, count 33->35, llms-full v2 marker) |
...
| W4.1 | B1 security scorecard | 🟢 DONE (Sep 6, beef366+2fcbaee) — heuristic v1 e2e: migration 0019 (score cols+partial index), internal/scorecard pure scorer (5 components, integer math, A–F bands, stdlib-only enforced, revenue-immune), store pgx+SQLite parity (Rescore*/GetScorecard, native-only guard, advisory hook), GET /v1/packages/{name}/scorecard + search "scorecard" field (explicit-null contract) + packument detail + non-fatal rescore on publish (token+OIDC), startup backfill + nightly 24h sweep (overlap guard); CLI: search SECURITY col, info Security line, audit SECURITY col+JSON (advisory-only, exit code untouched); reviewer APPROVE (atomic scored_at, sweep guard, warn logs); 9 docs updated; gates green both repos | app dev | pharos-registry beef366, pharos-cli 2fcbaee |
| W4.2 | D1 boost plumbing | 🟢 DONE (Sep 7, 0a2a37d..9c6df32 + 283372b..deab7a6) — registry: migration 0020 boosts table, store pgx+SQLite parity (CRUD, soft-delete, MaxBoostedSlots=2, BoostMatchesQuery conservative floor, ApplyBoosts pure merge), Search() post-cache boost append (cache stays organic-only), POST/GET/DELETE /v1/boosts (auth), search `boosted` array (always present, `[]` empty) + `sponsored` omitempty + boundary re-assert; CLI: search SPONSORED block above organic table (max 2, ` [boosted]` marker), organic output byte-identical when no boosts, --json `boosted` null→`[]` normalization; scorecard untouched (revenue-immune); docs: openapi (fixed pre-existing dupe path keys, 25→28) + REGISTRY_API §3.7 + SPEC §12.10.2 + CLI_REFERENCE + README×2 + PLAN; gates green both repos; ext adversarial review → 2 REQUIRED (F1 pgx ListBoosts arg-parity 500 on Postgres, F2 substring false-positive `ai`⊆`aim`) + 8 rec, all fixed (`4057445`+`9c6df32` reg, `deab7a6` cli; F3 id-DESC tiebreak, F5 prod-router 401s, F6-F10 UX+docs), reg tests incl. placeholder-parity + determinism + prod-router 401 | app dev | pharos-registry 0a2a37d+3c3cacf+751df73+4057445+9c6df32, pharos-cli 283372b+963149d+deab7a6 |
| W5.1 | A6 source-tracked updates | 🟢 DONE (Sep 7, 04f0113..c066ec4 + review-fix batch) — origin metadata (registry/adopted) recorded at install/adopt, update preserves additive fields + bumps registry Ref, pin/unpin + pinned-skip in update apply, update --check with origin-aware repo/changelog links (github/gitlab only, never fabricated), receipts carry no pinned annotation; ext adversarial review CHANGES-REQUESTED → 1 REQUIRED fixed (R1: re-adopt preserves installed Version/Integrity/Transport — pin desync + dead-registry Version wipe) + C1-C7 closed (C4 dep-install pin movement now documented contract, C5 repo_url trailing-slash normalization, C2/C3 receipt-semantics + legacy-backfill llmNotes, C6 scp-style/pin-JSON-stdout tests, C1 CLI_REFERENCE pin/unpin + --check in pharos-registry, C7 this row); docs/llm.txt regenerated; gates: build/vet/test, -race -count=2, GOOS=windows build all green | app dev | pharos-cli 04f0113..c066ec4 (+this fix) |
| W5.2 | C2 idle-cost budgets | 🟢 DONE (Sep 8, 8125c82..4d7e1b3) — pharos budget: read-only idle cost from daemon state (RSS via /proc Linux, tasklist CSV Windows w/ fallbacks), 30-day projection, JSON purity; NEW internal/procs (portable PID liveness+RSS); ext review CHANGES-REQUESTED → R1 fixed (budget was mutating via daemon Status() — deleting stale daemon.pid; now reads via new read-only exports, PID file survives all 4 live scenarios); regression tests corrupt-state/live-daemon; gates+race green | app dev | pharos-cli 8125c82..4d7e1b3 |
| W5.3 | A5 pharos expose (scoped) | 🟢 DONE (Sep 8, 4d7e1b3..d784360) — internal/expose: token-auth reverse proxy (constant-time compare, SHA-256 hash-at-rest 0600 store, TTL self-termination, lock contention loud-error, auth header stripped before forward); cmd: expose start/list/stop (--json + PHAROS_JSON=1); ext review CHANGES-REQUESTED → R-1 (stdout purity both modes) + R-2 (stale stop-file killed next tunnel) + R-3 (stop false-success on store error) fixed + C-1 listener timeouts + C-2/C-4 docs; 40+ tests incl. proven-fail-before-fix; live security matrix 401×4/200/502/TTL/store/stop; windows CI perm-bit assertion guarded per daemon precedent; CI green BOTH jobs (d784360) | app dev | pharos-cli 4d7e1b3..d784360 |
| W5.4 | A7-MCP pharos serve (MCP stdio) | 🟢 DONE (Sep 8, 63081a9..37d94a4) — internal/mcpserver: hand-rolled JSON-RPC 2.0 stdio server (zero new go.mod deps, mcpclient framing discipline); methods initialize (protocolVersion 2024-11-05, serverInfo pharos+cmd.Version), notifications/initialized (silent ack), tools/list, tools/call, ping; errors -32601 unknown method / -32602 bad params / -32700 parse / -32600 invalid request+batch-unsupported, tool failures = isError tool results (never crash, panic recovery), null-id requests treated as notifications, stdout purity (diagnostics → stderr only); tools search (limit clamp 1..50) / info (install_hint) / list_installed (legacy origin backfill "registry", clients never null) + install opt-in via --allow-install (omitted from tools/list when off, direct call → isError naming the flag; enabled = full install pipeline: semver resolve, store, canonical config w/ idle 60, detected clients, pharos.lock origin InstalledVia "pharos serve", dep resolution warnings); 25+ tests: in-process unit (protocol errors, purity, ctx-cancel, panic) + subprocess e2e (handshake, tools/list 3-vs-4, search/info vs httptest fixture, error codes, strict-JSON transcript purity, stdin-EOF shutdown w/ no extra frames); gates: build/vet/test, -race -count=2, GOOS=windows build+vet, GOOS=darwin build all green; ext review APPROVED (first first-pass of W5, 0 required findings; C-1 stdin comments / C-2 scanner-budget comment / C-3 install-success e2e → backlog riders); TRON live probe real binary 20/20 (purity, handshake, install gate, garbage-stdin survival); CI green ubuntu+windows | app dev | pharos-cli |
| W5   | A6 (W5.1), C2 (W5.2), A5 (W5.3), A7 (W5.4) — all four shipped | 🟢 DONE (Sep 8, W5.4 closed 37d94a4; all waves ext-reviewed, pushed, CI green) | app dev | pharos-cli |
| W6   | metrics + monetization | ⚪ parked (no selling before W4.1 + volume; PRE-W6 GATE: boost ownership scoping per W4.2 review F4 — IDOR-by-design accepted for D1, mandatory fix before purchase flow) | chris+app dev | — |
...
| site | Wave F: GSC 404 root-cause fix — getPackage tri-state contract (200->pkg, 4xx->null, 5xx/timeout->RegistryUnavailableError throw), 3-attempt retry, [...name]/error.tsx boundary. Transient failures no longer cached as 404 (was: s-maxage 3600 poisoned slugs during deploys; A11y reproduced 404-cached vs API 200). Build agent glm53-nothinking + TRON QA (tsc 0, build 97/97, live serve 200/404/200). Deployed bef304a, live-verified. GSC alerts self-heal as Googlebot recrawls | web dev | pharos-web bef304a |
