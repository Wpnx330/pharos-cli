...
| W4.2 | D1 boost plumbing | 🟡 BUILT, PENDING REVIEW (Sep 7, 283372b+751df73) — registry: migration 0020 boosts table, store pgx+SQLite parity (CRUD, soft-delete, MaxBoostedSlots=2, BoostMatchesQuery conservative floor, ApplyBoosts pure merge), Search() post-cache boost append (cache stays organic-only), POST/GET/DELETE /v1/boosts (auth), search `boosted` array (always present, `[]` empty) + `sponsored` omitempty + boundary re-assert; CLI: search SPONSORED block above organic table (max 2, ` [boosted]` marker), organic output byte-identical when no boosts, --json `boosted` null→`[]` normalization; scorecard untouched (revenue-immune); docs: openapi (fixed pre-existing dupe path keys, 25→28) + REGISTRY_API §3.7 + SPEC §12.10.2 + CLI_REFERENCE + README×2 + PLAN; gates green both repos | app dev | pharos-registry 0a2a37d+3c3cacf+751df73, pharos-cli 283372b |
