...
var llmNotes = map[string]llmNote{
	...
	"pharos expose": {
		output: "start: one-time token handoff — JSON: {name, addr, port, expiresAt, token} (single pure stdout document; progress to stderr); Plain: banner + token (shown once, stored only as a SHA-256 hash in ~/.pharos/expose.json) + ready-to-copy curl/client-header snippet + expiry line. Exit 1 (stderr guidance) when --addr is missing/invalid, --ttl is <=0 or >24h, the daemon is not running, the name is not daemon-managed, an expose for the name is already live, or (foreground only) the listen address is taken — with --background the parent exits 0 after the handoff and a taken port surfaces as a 'not confirmed yet' stderr warning (~5s) plus an ERROR line in ~/.pharos/expose.log",
